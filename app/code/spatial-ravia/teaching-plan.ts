...
import type { ScientificClaimId, ProvenanceSourceId } from "./scientific-fidelity-provenance.ts";
...
type ReferenceContext = { actors: Set<string>; groups: Set<string>; claims: Set<string>; sources: Set<string> };
function validateReference(value: unknown, path: string, context: ReferenceContext, issue: (path: string, message: string) => void): void {
  if (!keys(value, path, ["kind", "actorId", "groupId", "claimId", "sourceId"], issue)) return;
  const ref = value as UnknownRecord;
  const expected = ref.kind === "actor" ? "actorId" : ref.kind === "group" ? "groupId" : ref.kind === "claim" ? "claimId" : ref.kind === "source" ? "sourceId" : undefined;
  if (!expected) { issue(`${path}.kind`, "must be actor, group, claim, or source"); return; }
  ["actorId", "groupId", "claimId", "sourceId"].forEach((key) => { if (key !== expected && ref[key] !== undefined) issue(`${path}.${key}`, "does not match reference kind"); });
  if (!text(ref[expected])) { issue(`${path}.${expected}`, "is required"); return; }
  const collection = expected === "actorId" ? context.actors : expected === "groupId" ? context.groups : expected === "claimId" ? context.claims : context.sources;
...
export function validateTeachingPlan(plan: TeachingPlan, scene: ScientificSceneSpec): TeachingPlanValidationResult {
  const issues: TeachingPlanValidationIssue[] = [];
  const issue = (path: string, message: string) => issues.push({ path, message });
  if (!keys(plan, "plan", ["schemaVersion", "planId", "sceneId", "requestMode", "learningObjective", "prerequisiteAssumptions", "chapters", "narrationCues", "annotations", "causalSteps", "contrasts", "misconceptionCorrection", "projections"], issue)) return { valid: false, issues };
  const value = plan as unknown as UnknownRecord;
  if (value.schemaVersion !== "1") issue("plan.schemaVersion", "must be 1");
  ...
  const context: ReferenceContext = { actors: new Set(scene.actors.map((actor) => String(actor.actorId))), groups: new Set(scene.groups.map((group) => String(group.groupId))), claims: new Set((scene.claimIds ?? []).map(String)), sources: new Set(scene.fidelityProvenance.sources.map((source) => String(source.sourceId))) };
  const chapters = list(value.chapters, "plan.chapters", issue);
  const chapterIds = new Set<string>();
  chapters.forEach((chapter, index) => {
    const path = `plan.chapters[${index}]`;
    if (!keys(chapter, path, ["chapterId", "order", "title", "focus", "context", "narrationCueIds", "annotationIds", "causalStepIds", "contrastIds"], issue)) return;
