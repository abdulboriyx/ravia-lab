...
test("formal scientific evaluation suite covers at least 100 cases and required dimensions", () => {
  ...
  assert.ok(spatialRaviaEvaluationCases.length >= 100);
  ...
  assert.ok(categories.has("missing-parameter"));
...
test("formal scientific evaluation produces machine and human-readable analysis", () => {
  ...
  assert.ok(report.totalCases >= 100);
